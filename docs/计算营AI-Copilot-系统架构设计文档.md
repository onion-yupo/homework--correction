...
- 家长提交作业图片 → 腾讯云 AI 批改 → 老师复核（确认/修改判定、人工标注）→ AI 生成反馈建议（六模块：整体情况反馈、书写评价、问题分析、整体改进计划、给家长的小提示、加练题目）→ 老师编辑调整后发给家长
...
| 模块            | 优先级 | 说明            |
| ------------- | --- | ------------- |
| 数据底座（SQLite）  | P0  | 所有后续功能的基础     |
| 工作台首页（任务追踪矩阵） | P0  | 老师日常最高频使用的页面  |
| 历史数据批量导入      | P1  | 让底座有真实数据      |
| 批改详情页改造       | P1  | 面包屑、上下学生切换    |
| 学情分析页面        | P2  | 个人学情 + 全班概览教研增强（6 模块） |
| 全局导航 + 老师身份   | P1  | 系统级体验升级       |
| 周报生成 + PDF 导出  | P2  | 已实现，含大拇指评价体系  |
| 学期总结视频生成       | P2  | 已实现，Remotion + TTS + LLM 旁白 |
...
- **人工标注**：进入框选模式，老师可框选 AI 未识别的题目并标记对/错
- **全部隐藏/全部显示**：纯视觉切换，临时隐藏所有标注方便查看原图，不影响数据统计和持久化
- **导出图片**：使用 html2canvas 截取当前带标注的作业原图（包含系统标注 + 人工标注，已隐藏的不导出），优先复制到剪贴板，不支持时自动下载 PNG
...
1. **班课数据总览**：顶部 6 格统计卡片（参与学生数 / 总提交天次 / 总题数 / 平均正确率 / 人均完成天数 / 满分天次），复用 `class-overview` 返回数据在前端聚合
2. **每日正确率趋势（增强）**：柱状图 + 全班平均线（虚线） + 难度异常天红色标注（低于平均 15%+）+ 每柱上方显示参与人数。柱体区域和标签区域分离，避免文字与图形重叠
3. **高频错题（全量）**：按题目（`question_title + correct_answer + day`）聚合全班错误率。后端 `class-error-questions` API 返回全量错题。默认显示 TOP 10，底部"展开全部 X 道错题"按钮查看全部。每道错题可展开查看做错学生名单（姓名 + 学生答案）+ 作业原图缩略图（通过 `/api/correction/image/:jobId` 加载）
4. **知识点掌握全景**：将 `class-knowledge-top?limit=all` 返回的全部知识点分为四层（薄弱 >50% / 需加强 20-50% / 良好 5-20% / 完全掌握 <5%），每层用不同颜色条（红/橙/蓝/绿），显示涉及学生数。完全掌握层折叠为标签云
5. **学生正确率排行（增强）**：表格 + 满分之星⭐（正确率 TOP 3）+ 坚持之星💪（完成天数 TOP 3）+ 全班中位数线，低于中位数的行半透明
6. **AI 教研建议**：进入页面自动加载已保存建议或自动生成。基于全班统计概要 + 薄弱知识点 TOP 5 + 高频错题 TOP 5 + 难度异常天，调用豆包 2.0 lite 流式生成 3-5 条教研优化建议。生成后自动持久化到 `reports` 表（`student_id='__class__'`, `report_type='class_advice'`）。支持补充指令注入重新生成（与反馈建议、提醒文案逻辑一致）
...
报告采用 A4 分页设计，视觉风格遵循 DESIGN.md（洋葱蓝色调 + Alibaba PuHuiTi 2.0 字体 + 洋葱学园 Logo）。内容基于老师实际周报模板改造，分为 5 个模块：

1. **基本信息** — 5 格卡片（统计周期、练习天数、题目总数、正确数、总体正确率）+ 每周进步速览表格（天/题数/正确/错误/正确率，四色标签）
2. **精彩表现** — 合并展示 4 个子模块：
   - 本周亮点（星标列表，LLM 生成）
   - 掌握的知识点（文本列表，LLM 基于数据生成）
   - 值得称赞的细节（圆点列表，LLM 生成）
   - 书写评价（评价图标卡片 + 老师点评，从反馈建议中提取）
3. **易错汇总** — 编号橙色卡片（序号 1/2/3...），每题展示题目 + 💡 学习小技巧（LLM 生成）
4. **学习建议** — 三段式：💪 鼓励话语 + 🌱 成长空间 + 📝 练习建议（LLM 生成）
5. **老师备注** — 可选手动填写区

#### 封面

洋葱蓝渐变封面，周次数字为最大视觉元素，显示洋葱学园 Logo + 学期·周次·学生姓名。

#### LLM Prompt 设计

后端 prompt 使用分段式 section markers（`===HIGHLIGHTS===`、`===KNOWLEDGE_TEXT===`、`===PRAISE===`、`===WRITING===`、`===ERROR_TIPS===`、`===ENCOURAGEMENT===`、`===GROWTH===`、`===PRACTICE===`、`===REMARK===`），前端按 marker 解析各段内容。支持向后兼容旧格式 marker。
...
2. 服务端 `buildReportHtml()` 生成完整独立 HTML（内嵌 CSS + Alibaba PuHuiTi 2.0 字体 + base64 Logo）
3. Puppeteer 浏览器单例（60s 空闲自动关闭）→ `page.setContent(html)` → `page.pdf()` → A4 PDF
4. 返回 PDF 文件流供前端下载

不依赖前端服务。字体从本地文件读取并 base64 嵌入 HTML，Logo 同样 base64 内联，确保任何环境下渲染一致。

#### 稳定性保障

- **Puppeteer 浏览器单例**：避免每次导出启动新 Chrome 进程，减少 CPU/内存冲击
- **SQLite busy_timeout = 5000ms**：PDF 渲染期间的并发数据库查询不会立即失败
- **前端 fetchRetry**：关键数据加载（matrix、teachers、enrollments）自动重试 3 次

#### 设计规范（DESIGN.md）

基于洋葱学园品牌视觉规范，参考 awesome-design-md 格式生成的设计系统文档：
- **主色调**：洋葱蓝（#1B6AFF），辅助蓝（#2B7FFF / #5B9CFF / #E8F0FE）
- **字体**：Alibaba PuHuiTi 2.0（Regular/Medium/Bold/Heavy 四字重）
- **Logo**：洋葱学园官方标识，白色和彩色两版
- 覆盖范围：周报（前端 + PDF）、班课总结视频
...
| `/api/dashboard/remind`     | POST | 生成提醒家长交作业文案（流式，调用豆包 2.0 lite）                            |
...
| `/api/analytics/class-overview`           | GET    | 全班概览（学生列表 + 趋势 + 满分天次）。参数：`semester`, `day`(可选)      |
| `/api/analytics/class-knowledge-top`      | GET    | 全班知识点（支持 `limit=all` 返回全部，含涉及学生数）。参数：`semester`, `limit` |
| `/api/analytics/class-error-questions`    | GET    | 全班高频错题（按题目聚合，含做错学生列表 + job_id）。参数：`semester`, `limit` |
| `/api/analytics/class-advice`             | GET    | 加载已保存的全班教研建议。参数：`term`, `semester`                        |
| `/api/analytics/class-advice/save`        | POST   | 保存全班教研建议到 reports 表                                            |
| `/api/analytics/class-teaching-advice`    | POST   | AI 教研建议流式生成（豆包 2.0 lite，输入结构化班课数据，支持补充指令注入）   |
...
| 接口                            | 方法   | 说明                                                                     |
| ----------------------------- | ---- | ---------------------------------------------------------------------- |
| `/api/report/generate-weekly` | POST | 聚合周数据 + 大拇指算法 + 豆包 2.0 lite 流式生成周报文案。支持 `instruction` 补充指令。先发 `__DATA__` 结构化 JSON，再流式输出文本 |
...
### 6.3 每日反馈建议 — LLM Prompt 设计

反馈建议由豆包 2.0 lite（多模态，同时接收作业图片和批改结构化数据）流式生成，按 `### 标题` 切分为六个独立模块，每个模块支持独立编辑、复制、导出图片、重新生成（可注入补充指令）。

#### 六模块结构

| 序号 | 模块 | 输出格式 | 说明 |
|------|------|----------|------|
| 一 | 整体情况反馈 | 自然段落（无 bullet） | 正向总结 + 数据统计 |
| 二 | 书写评价 | 三行维度评价 + 段落评语 | 基于图片观察：整体书写 / 字迹清晰度 / 卷面整洁度 |
| 三 | 问题分析 | 三层结构（加粗标题→属性行→编号步骤） | （一）知识点类错误 +（二）计算习惯与过程规范 |
| 四 | 整体改进计划 | 加粗标题 + 编号列表 | 每日核心目标 / 3分钟自查步骤 / 针对性练习建议 |
| 五 | 给家长的小提示 | 自然段落 | 可直接发给家长的温和沟通文字 |
| 六 | 加练题目 | 三级难度 × 编号列表 | 基础巩固 / 巩固 / 挑战，含【推荐】标记 |

#### "问题分析"模块的三层结构

以老师提供的反馈模板为参考，严格执行无 bullet 的分层排版：

```
第一层：**知识点：整千数减三位数（连续退位）**      ← 加粗独立标题行
第二层：错题数量：2道                              ← 属性行，每行独占，空行分隔
       共性错因：被减数中间0被借位后未按9计算
       典型错题示例：1000-569，百位0被十位...
第三层：▶ 解题步骤示例（整千数减三位数）：          ← 编号步骤
       1. 列竖式：1000 - 569
       2. 个位：0-9不够，向十位借...
       3. 个位：10-9=1
       ...
...
- **禁止 bullet 列表**：模块一至五的输出内容一律禁止 `- ` 或 `•` 列表符号
- **禁止思考过程**：严禁输出"让我想想""嗯不对""那我们还是按…"等推理/自我对话文字
- **禁词规则**：严禁"粗心""马虎""不认真"等笼统归因，只描述计算行为与知识点问题
- **知识点规范**：必须是具体题目考察类型（如"连续退位减法""乘法口诀记忆"），严禁"计算失误"等模糊词
- **属性字段独占一行**：错题数量、共性错因、典型错题示例不得合并到同一行

#### 模块导出图片

每个非加练模块（一至五）支持"导出图片"功能：
- 使用 html2canvas 截取模块内容，标题使用去掉序号前缀的纯文本（如"整体情况反馈"而非"一、整体情况反馈"）
- 优先复制到剪贴板，不支持时自动下载 PNG
- 文件名格式：`{学生名}_{模块标题}.png`

#### 书写评价与周报的数据链路

每日反馈的"二、书写评价"模块采用结构化三维度输出（整体书写/字迹清晰度/卷面整洁度），周报生成时通过正则 `### 二、书写评价` 精确提取该模块内容，作为周报"书写评价"部分的直接数据源。旧格式反馈（无独立书写模块）降级为截取 feedbackContent 前 500 字。
...
server/
  ...
  │   │   ├── vision.ts           // 现有（保持不变）
  │   │   ├── dashboard.ts        // 新增：工作台 API（overview/matrix/freeze/remind/sort-prefs）
  │   │   ├── analytics.ts        // 新增：学情分析 API（趋势/知识点/错题/全班概览）
  │   │   ├── teachers.ts         // 新增：老师管理 API
  │   │   ├── report.ts           // 新增：周报生成 + PDF 导出（流式 LLM + Puppeteer）
  │   │   └── import.ts           // 新增：批量导入接口
  │   └── index.ts                // 入口（注册新路由）
  └── scripts/
...
| 设计系统      | DESIGN.md（洋葱学园视觉规范，参考 awesome-design-md）                |
...
**Step 1 — 数据加载**：从 `homework-correction/server/data/copilot.db` 读取学生的 jobs、answers、reports 数据，计算 dailyData、knowledgePoints、thumbsUp 等。`highlights`（本期亮点）不再拼接周报原文，而是从整期统计数据中直接提炼 3-5 条简洁亮点（满分天数、出勤情况、知识点掌握度、整体正确率、题量），风格参考周报"值得称赞的细节"。`teacherComment` 优先取整期总结报告，其次取最近一期周报的 SUMMARY。
